...
from db_functions import convert_db_info_to_dict_noderank
...
class NoderankData:
    ...
    def __init__(self, mode, case, case_id):
        ...
        #f.close()
        self.caseconfig = convert_db_info_to_dict_noderank(case_id)
        # Get scenarios
...
#            settings = {}
        ...
        self.connections_used = scenario_config["use_connections"]
        ...
        #if scenario_config["use_bias"]:
        #    self.bias_used = settings["use_bias"]
        #else:
        #    self.bias_used = False
        self.bias_used = scenario_config["use_bias"]

        #if "dummies" in settings:
        #    self.dummies = settings["dummies"]
        #else:
        #    self.dummies = False
        self.dummies = scenario_config['dummies']
    ...
    def get_boxes(self, scenario, datadir, typename):
        if self.caseconfig[scenario]["boxindex_type"] == "range":
            boxindexes = range(
                        self.caseconfig[scenario]["boxindex_start"],
                        self.caseconfig[scenario]["boxindex_end"] + 1,
                    )
...
def noderankcalc(mode, case, case_id, writeoutput, preprocessing=False):
    ...
    """
    noderankdata = NoderankData(mode, case, case_id)
